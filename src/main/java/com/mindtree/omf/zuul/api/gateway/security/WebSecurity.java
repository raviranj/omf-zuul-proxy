 ...
 * 
 * @Autowired public WebSecurity(Environment environment) { this.environment =
 * environment; }
 * 
 * @Override protected void configure(HttpSecurity http) throws Exception {
 * 
 * http.csrf().disable(); http.headers().frameOptions().disable();
 * http.authorizeRequests().antMatchers(environment.getProperty(
 * "api.users.actuator.url.path")).permitAll()
 ...
 * .antMatchers(HttpMethod.POST,
 * environment.getProperty("api.login.url.path")).permitAll().anyRequest()
